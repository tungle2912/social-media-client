...
import { ConversationMessageStatus, ConversationType, MESSAGE_TYPE, TYPE_DELETE_MESSAGE } from '~/definitions/models/message';
...
const MessageItem = ({
  ...
  function convertLinksToAnchors(message: any): string | TrustedHTML {
  ...
  return (
    <div
      ...
      className={classNames(
        `mt-4 flex ${!message?.user ? 'justify-center' : message?.owner ? 'justify-end' : 'justify-start'} items-center`,
        styles.msg
      )}
    >
      ...
      {message?.user ? (
        message?.owner ? (
          ...
          <Flex className="w-full">
            <div className="mr-2 relative cursor-pointer" onClick={() => {}}>
              {message?.user?.basicPersonalInfo?.profilePhoto ? (
                <Avatar
                  src={message?.user?.basicPersonalInfo?.profilePhoto || ''}
                  alt={t('avatar')}
                  className={styles.avatar}
                />
              ) : (
                <div className={styles.avatar}>{getUsername(message?.user?.basicPersonalInfo, true)}</div>
              )}
              {message?.isOnline && (
        ...
        <div className="px-[16px] text-sm max-w-[100%] py-[6px] rounded-[10px] bg-[#F7F7F7] text-[#333333] flex items-center">
          {message.additionalData?.basicPersonalInfo?.profilePhoto ? (
            <Avatar
              src={message.additionalData?.basicPersonalInfo?.profilePhoto || ''}
              alt={t('avatar')}
              className={styles.avatarUserIcon}
            />
          ) : (
            <div className={styles.avatarUserIcon}>{getUsername(message?.additionalData?.basicPersonalInfo, true)}</div>
          )}
          <i className="text-xs">{message?.message}</i>
        </div>
      )}
    </div>
  );
